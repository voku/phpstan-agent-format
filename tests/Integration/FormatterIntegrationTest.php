...
final class FormatterIntegrationTest
{
    public static function run(): void
    {
        ...
        /** @var array{tool: string, summary: array{totalIssues: int}, clusters: list<array{representativeIssues: list<array{symbolContext: array{parameterName: ?string, expectedType: ?string, inferredType: ?string, typeOrigin: ?string}, secondaryLocations: list<array{file: string, line: int}>, contextTrace: array{hops: list<array{summary: string}>}}>}>} $decoded */
        ...
        foreach ($issue['contextTrace']['hops'] as $hop) {
            if (str_contains($hop['summary'], 'PHPDoc') || str_contains($hop['summary'], 'AST node')) {
